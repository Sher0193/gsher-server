...
class Router {
    ...
    login(app, db) {
        app.post('/login', (req, res) => {
            ...
            db.query('SELECT * FROM users WHERE name = ? LIMIT 1', cols, (err, data, fields) => {
                    if (err) {
                        res.json({
                            success: false,
                    ...
                    if (data && data.length === 1) { // user found
                        bcrypt.compare(password, data[0].password, (brcryptErr, verified) => {
                            if (verified) {
